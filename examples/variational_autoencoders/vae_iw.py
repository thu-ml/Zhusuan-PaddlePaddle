...
class Generator(BayesianNet):
    def __init__(self, x_dim, z_dim, n_particles):
        super().__init__()
        self.x_dim = x_dim
        self.z_dim = z_dim
    ...
    def forward(self, observed):
        ...
        #assert(sample_x.shape[1] == batch_len)
        return self
...
def main():
    ...
    for epoch in range(epoch_size):
        for step in range(num_batches):
            ...
            ##loss= model(x)
            loss= model({'x':x})
            assert(generator.log_joint().shape[0] == lb_samples)
    ...
    sample = cache['x_mean'][0][0].numpy()

    cache = generator({}).cache
    sample_gen = cache['x_mean'][0][0].numpy()
    ...
    save_img(batch_x, os.path.join(result_fold, 'origin_x_iw.png' ))
    save_img(sample,  os.path.join(result_fold, 'reconstruct_x_iw.png' ))
    save_img(sample_gen,  os.path.join(result_fold, 'sample_x_iw.png' ))
